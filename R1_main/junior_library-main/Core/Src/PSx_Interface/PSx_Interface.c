 ...
 * Function Example		: PSxMasterInit(&ps4,&hi2c1);
 */
void PSxMasterInit(PSxBT_t *psxbt,I2C_HandleTypeDef* hi2cx,DMA_HandleTypeDef* hi2cx_rx_dma){
	psxbt->hi2cps4 = hi2cx;
	psxbt->hi2cps4_rx_dma = hi2cx_rx_dma;
	psxbt->master=1;
	I2CX_DMA_RX_Init(psxbt->hi2cps4, psxbt->hi2cps4_rx_dma, main_board_1, CLOCK_SPEED_400KHz);
	HAL_I2C_Master_Receive_DMA(psxbt->hi2cps4, (0x44 << 1) ,(unsigned char *)psxbt->ReceiveBuffer, 11);
}

/*
 * Function Name		: PSxSlaveInit
 * Function Description : This function is called to initialize PSx.
 * Function Arguments	: *psxbt	, Pointer to structure PSxBT_t
 * 						   hi2cx    , Pointer to I2C handle
 * Function Return		: None
 * Function Example		: PSxSlaveInit(&ps4,&hi2c1);
 */
void PSxSlaveInit(PSxBT_t *psxbt,I2C_HandleTypeDef* hi2cx){
	psxbt->hi2cps4 = hi2cx;
	psxbt->slave=1;
	I2CxInit(psxbt->hi2cps4, main_board_1, CLOCK_SPEED_400KHz,ENABLE);
	HAL_I2C_Slave_Receive_IT(psxbt->hi2cps4 ,(unsigned char *)psxbt->ReceiveBuffer, 11);
}

/*
 * Function Name		: PSx_MasterHandler
 * Function Description : This function is called to handle the received PSx data from ESP32.
 * Function Remarks		: None
 * Function Arguments	: *psxbt	,pointer to structure PSxBT_t
 * Function Return		: None
 * Function Example		: PSx_MasterHandler(&ps4);
 */

void PSx_MasterHandler(PSxBT_t *psxbt){
	psxbt->disconnected=0;
	HAL_I2C_Master_Receive_DMA(psxbt->hi2cps4, (0x44 << 1) ,(unsigned char *)psxbt->ReceiveBuffer, 11);
	if(psxbt->ReceiveBuffer[0]==0x02&&(psxbt->ReceiveBuffer[10]=='c'||psxbt->ReceiveBuffer[10]=='d')){
...
/*
 * Function Name		: PSx_SlaveHandler
 * Function Description : This function is called to handle the received PSx data from ESP32.
 * Function Remarks		: None
 * Function Arguments	: *psxbt	,pointer to structure PSxBT_t
 * Function Return		: None
...
void PSxConnectionHandler(PSxBT_t *psxbt) {
	if (psxbt->slave) {
		if (psxbt->disconnected) {
			...
			psxbt->joyR_2 = 0;
			psxbt->joyL_2 = 0;
			HAL_I2C_DeInit(psxbt->hi2cps4);
			I2CxInit(psxbt->hi2cps4, main_board_1, CLOCK_SPEED_400KHz, ENABLE);
			HAL_I2C_Slave_Receive_IT(psxbt->hi2cps4 ,(unsigned char *)psxbt->ReceiveBuffer, 11);
		}
		psxbt->disconnected = 1;
	} else if (psxbt->master) {
		if (psxbt->disconnected) {
			psxbt->button = 0;
			psxbt->joyR_y = 0;
			psxbt->joyR_x = 0;
			...
			psxbt->joyR_2 = 0;
			psxbt->joyL_2 = 0;
			HAL_DMA_DeInit(psxbt->hi2cps4_rx_dma);
			HAL_I2C_DeInit(psxbt->hi2cps4);
			I2CX_DMA_RX_Init(psxbt->hi2cps4, psxbt->hi2cps4_rx_dma, main_board_1, CLOCK_SPEED_400KHz);
			if ((HAL_I2C_IsDeviceReady(psxbt->hi2cps4, (uint16_t) (0x44 << 1),3, 5) == HAL_OK)) {
				HAL_I2C_Master_Receive_DMA(psxbt->hi2cps4, (0x44 << 1),(unsigned char*) psxbt->ReceiveBuffer, 11);
				psxbt->disconnected = 0;
			}
			psxbt->disconnected = 1;
